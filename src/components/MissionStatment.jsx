...
const MissionStatment = ({ language }) => {
  const lines = ABOUT_TEXT[language].trim().split('\n'); // Get text in selected language
  const bulletPoints = lines.slice(0); // Convert lines into bullet points
        ...
        >
          <h2 className="my-20 text-5xl font-semibold">
            <span className="text-orange-200">
              {language === "en" ? "Mission" : "Énoncé de"}
            </span> 
            {" "}
            {language === "en" ? "Statement" : "Mission"} 
          </h2>
